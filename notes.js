...
Since switching to quiz-maker

Incorporated variable for main background color and hover color on css

Changed style of homePage

Easy fix made for holding render of dashboard quizzes and ViewQuiz quiz data until
state returned. To fix that gross load for a sec of the skeleton and delete button.
...
I think for 'quiz-maker' version we should keep all the variables like teacher, lastName,
but send them to db as null....or should I use entirely new db? Or new collection?

Kind of using cheats in the render of Dashboard and ViewQuiz. Particularly in ViewQuiz.
In ViewQuiz, when we want to render, we're checking to see if state and state.scores exists
and isn't null in order to wait until it is to render. This keeps us from getting that
gross looking div skeleton and button loading a sec before the data. The reason I say 
it's a cheat is b/c we dont' actually use state.scores until we're switching to the scores
ux, but the reason I know it works is it's a state that is created and populated when
we do the general server call and data grab for this component, so it works. Maybe the
'correct' way would be make a state.dataRecieved and set it true or false at end of 
data call, but...w/e
