...
class FindQuizComponent extends React.Component {
    ...
    getAllQuizzes(e) {
        let self = this;
        let data = {};
        axios.post('/getAllQuizzes', data).then(function(response) {
            let quizOptions = response.data.quizOptions;
            let optionsRender;
                ...
                optionsRender = quizOptions.map(function(ele) {
                    return (
                        <div key={ele._id} onClick={() => self.takeQuiz(ele._id)}>
                            <div className="neo_quiz_list_item_container">{ele.title}</div>
                        </div>
                        );
                });
            }
        ...
        return (
            <div>
                ...
                <div className="form_page_container">
                    {this.state && this.state.quizOptions && <div className="neo_form_container">
                        <p className="neo_form_container_title">Choose a quiz</p>
                    ...
                    <div className="neo_form_container">
                        {this.state.page == 1 ? <p className="neo_form_container_title">Enter School and Teacher below</p> : <p className="neo_form_container_title">Choose a quiz to take</p>}
                        <div className="neo_form">
                            <div className="neo_form_inputs_container">
                                {this.state.page == 1 ?
                                    <form onSubmit={this.getQuizzes}>
                                        <div>
                                            <input name="school" value={this.state.school} onChange={this.schoolHandler} placeholder="school" autoComplete="off" required autoFocus></input>
                                        </div>
                                        <div>
